...
export async function GET(request: NextRequest) {
    try {
        ...
        const url = new URL(`${BACKEND_URL}/admin/questions/pending`);
        ...
        if (!response.ok) {
            const error = await response.json().catch(() => ({ message: 'Failed to fetch pending questions' }));
            return NextResponse.json(
                { error: error.message || 'Failed to fetch pending questions' },
