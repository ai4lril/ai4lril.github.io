...
export class StorageService {
  ...
  /**
   * Upload a file to MinIO storage (for exports, etc.)
   * @param filePath - Path to the file on disk
   * @param objectName - Name for the stored file in MinIO
  ...
  async uploadFile(
    ...
    try {
      ...
      const port = process.env.MINIO_PORT || '9000';
      const endpoint = process.env.MINIO_ENDPOINT || 'localhost';
      const url = `${protocol}://${endpoint}:${port}/${this.bucketName}/${objectName}`;
      return url;
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
